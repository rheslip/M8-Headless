...
It's relatively cheap to build (~ $150) but it requires serious electronic building skills and experience configuring Linux.
...
I wired up a battery gauge using two MIC803-30, a MIC803-31 (what I had) and a resistor divider chain. These pull PI gpio pins 17, 27 and 22 low in sequence as the battery voltage drops. I modded the TFT driver low battery code to draw four different battery icons based on the GPIO inputs. The modded code is here as an example of tweaking the TFT driver but it's pretty specific to the circuit I used.

---

## Enclosure

There is 3D printable enclosure in OpenSCAD for this project. It's still a work in progress. It's a bit taller, wider and thicker than the M8. The enclosure needs lots of ventilation - heatsinks were added to the Teensy and the Pi. The Pi overheat icon appears on the display sometimes when there is a lot of screen activity and CPU use.
...
You will want to configure your Pi display for 640x480 so it downscales nicely to 320x240 on the TFT. If the TFT display looks fuzzy check LXDE antialiasing may be turned on - see the O/S configuration notes below.
...
My notes on how to build the image - NOT a shell script, it's just my way of making notes:
...
- Uncomment the `hdmi_force_hotplug` line so it always starts the display code
- Uncomment the two HDMI resolution lines
...
dtoverlay=gpio-key,gpio=5,active\_low=1,gpio\_pull=up,label=down,keycode=108
dtoverlay=gpio-key,gpio=6,active\_low=1,gpio\_pull=up,label=up,keycode=103
dtoverlay=gpio-key,gpio=13,active\_low=1,gpio\_pull=up,label=left,keycode=105
dtoverlay=gpio-key,gpio=19,active\_low=1,gpio\_pull=up,label=right,keycode=106
dtoverlay=gpio-key,gpio=26,active\_low=1,gpio\_pull=up,label=shift,keycode=42
dtoverlay=gpio-key,gpio=16,active\_low=1,gpio\_pull=up,label=enter,keycode=31
dtoverlay=gpio-key,gpio=20,active\_low=1,gpio\_pull=up,label=option,keycode=30
dtoverlay=gpio-key,gpio=21,active\_low=1,gpio\_pull=up,label=play,keycode=57
...
That's it for changes to `/boot/config.txt`

---

### Install tools needed to build the TFT driver:
...
```bash
cmake -DSTATISTICS=0 -DARMV6T2=0 -DARMV7=1 -DGPIO_TFT_DATA_CONTROL=25 -DGPIO_TFT_RESET_PIN=24 -DGPIO_TFT_BACKLIGHT=18 -DSTATISTICS=0 ..
make -j4
make install
```
...
* If you get the TFT display updating, congratulations! Hit ctrl-C to stop it.

---

### Install `fbcp-ili9341` to run on startup:
...
## Other Linux configuration notes

* I set the following in `~/.config/lxsession/LXDE-pi/autostart`:
...
* If you get a fuzzy display check if antialiasing is enabled in the LXDE settings. Turn off anti-aliasing for the best picture.
...
## Using the M8C Headless Firmware

Flash your Teensy 4.1 with the M8 headless firmware from the Dirtywave repo: [https://github.com/dirtywave/m8-headless](https://github.com/dirtywave/m8-headless)

Use the USB OTG cable to connect Teensy USB port to the Pi's USB OTG port.

Boot the Pi, the TFT display will show the M8 headless UI.

---

## Notes

This is a work in progress. Bugs and issues will be addressed over time.
...
Thanks to Dirtywave for the great tracker and inspiration.
